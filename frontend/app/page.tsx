...
export default function CineMatchHybrid() {
  ...
  return (
    <main className="min-h-screen bg-zinc-50 text-zinc-900 font-sans selection:bg-black selection:text-white">
      <nav className="w-full max-w-7xl mx-auto p-6 flex justify-between items-center z-50 relative">
        ...
        <div className="flex bg-zinc-100 p-1 rounded-full border border-zinc-200">
            <button 
                ...
                className={`px-4 py-1.5 rounded-full text-sm font-bold transition-all flex items-center gap-2 ${mode === 'search' ? 'bg-white shadow-sm text-black' : 'text-zinc-500 hover:text-black'}`}
            >
                <Search size={14}/> Search
            </button>
            <button 
                onClick={() => setMode("wizard")}
                className={`px-4 py-1.5 rounded-full text-sm font-bold transition-all flex items-center gap-2 ${mode === 'wizard' ? 'bg-black shadow-sm text-white' : 'text-zinc-500 hover:text-black'}`}
            >
      ...
      <div className="w-full max-w-7xl mx-auto px-6">
          {mode === "search" ? (
              <SearchMode onSelect={(m) => setSelectedResult(m)} />
          ) : (
              <WizardMode onSelect={(m) => setSelectedResult(m)} />
          )}
      </div>
...
function SearchMode({ onSelect }: { onSelect: (m: any) => void }) {
    ...
    return (
        <motion.div initial={{opacity:0}} animate={{opacity:1}} className="flex flex-col items-center pt-20">
            <h2 className="text-6xl font-black mb-6 tracking-tighter text-center">Find it fast.</h2>
            ...
            {!loading && results.length > 0 && (
                 <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 pb-20 w-full">
                    {results.map((item) => (
                        <div key={item.id} onClick={() => onSelect(item)} className="cursor-pointer">
                            <DirectionAwareHover imageUrl={item.poster || ""}>
                                <div className="space-y-1">
                                    <p className="font-bold text-xl leading-tight">{item.title}</p>
    ...
    return (
        <div className="w-full pt-10">
            <AnimatePresence mode="wait">
                ...
                {step === 'intro' && (
                    <motion.div key="intro" initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} exit={{opacity:0, y:-20}} className="max-w-xl mx-auto space-y-8">
                        <div className="text-center">
                            <h2 className="text-5xl font-black mb-2">The Concierge.</h2>
                            <p className="text-zinc-500">Let us curate a playlist for your current vibe.</p>
                        </div>
                        <div className="space-y-6 bg-white p-8 rounded-2xl border border-zinc-100 shadow-xl shadow-zinc-200/50">
                            <div>
                ...
                {step === 'loading' && (
                    <motion.div key="loading" exit={{opacity:0}}><MovieLoader /></motion.div>
                )}
                ...
                {step === 'select' && (
                    <motion.div key="select" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="pb-20">
                        <div className="flex justify-between items-end mb-8">
                            <div>
                                <h2 className="text-3xl font-bold">Refine the vibe.</h2>
                                <p className="text-zinc-500">Select 3 {genre} titles you enjoyed.</p>
                            </div>
                            <Button onClick={handleGetResults} disabled={selectedMovies.length < 1} size="lg" className="px-8">View Results <ArrowRight size={16}/></Button>
                        </div>
                ...
                {step === 'results' && (
                    <motion.div key="results" initial={{opacity:0}} animate={{opacity:1}} className="pb-20">
                         <div className="flex justify-between items-center mb-8">
                            <h2 className="text-3xl font-bold">Top Picks for {name}</h2>
